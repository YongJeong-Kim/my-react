...
//		messageTemplate.convertAndSendToUser("ccc", "/topic/chat", chat);
	...
	@MessageMapping("/snackbar/{toUsername}")
	@SendTo("/user/{toUsername}/queue/snackbar") // working subscribe /user/queue/snackbar
	public Chat ddss(@DestinationVariable String toUsername, Chat chat) {
		System.out.println("subscribe" + toUsername);
//		messagingTemplate.convertAndSendToUser(toUsername, "/queue/snackbar", chat);
		return chat;
	}

	@GetMapping("/dfdf")
	public @ResponseBody String dfdf() {
		return "dfdf";
	}
/*	@MessageMapping("/fff")
	...
	public ResponseEntity<?> someAction(@PathVariable String target) {
		messagingTemplate.convertAndSendToUser(target, "/queue/notify", "hi");
